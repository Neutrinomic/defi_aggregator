...
import {Observable; Subject; of;mapAsync; bufferTime; buffer; delay; null_func; pipe2; pipe3; pipe4; first; map; concatAll; mergeMap; distinct; reduce; takeUntil} "mo:rxmo";
...
import CMC "./services/cmc";
import Orally "./services/orally";
...
actor Aggregate {
    ...
    type SnsConfig = {
        ...
        other_treasuries: [{token_id: TokenId; owner: Principal; subaccount: [Nat8]}];
    };
    ...
    type TokenPriceOutput = {price:Float; buydepth2:Float;buydepth8:Float; buydepth50:Float; volume:Float};
    ...
    private func last_oracle_value(key :Text) : ?Float {
        ...
        let ?rez = Array.find(oracle_latest, func(x:(Text, Float)) : Bool {
            x.0 == key;
        }) else return null;
    ...
    public query func get_pairs(f: Frame, ids:[Nat], from:?Time.Time, to:?Time.Time) : async Result.Result<{data:[TickShared]; first: Time.Time; last:Time.Time; updated:Time.Time}, GetError> {
        let ft = first_tick;
        let (ticks, tsec) = f2t(f);
        ...
        let data = switch(ids.size() == 0) {
            case (true) {
                Array.tabulate<TickShared>(ticks_asked, func (i) : TickShared {
            ...
            case (false) {
                Array.tabulate<TickShared>(ticks_asked, func (i) : TickShared {
                    ...
                    Array.tabulate(ids.size(), func(idx:Nat) : ?TickItem {
    ...
    public query func get_tokens(ids:[Nat], from:?Time.Time, to:?Time.Time) : async Result.Result<{data:[TokenTickShared]; first: Time.Time; last:Time.Time; updated:Time.Time}, GetError> {
        let ft = first_tick;
        ...
        let data = switch(ids.size() == 0) {
            case (true) {
                Array.tabulate<TokenTickShared>(ticks_asked, func (i) : TokenTickShared {
            ...
            case (false) {
                Array.tabulate<TokenTickShared>(ticks_asked, func (i) : TokenTickShared {
                    ...
                    Array.tabulate(ids.size(), func(idx:Nat) : ?TokenTickItem {
                        if (ids[idx] >= tick.size()) return null;
                        tick[ids[idx]];
                    });
                });
    ...
    private func put_tick(ticks: Vector.Vector<Tick>, currentTick:Nat, (pairid, lastbid, lastask, volume, depthBid50, depthAsk50) : TickLast) : () {
        switch(Vector.getOpt<Tick>(ticks, currentTick)) {
            ...
            case(?p) { // update
                ...
                var psize = p.size();
                if (pairid >= psize) { // recreate larger array
        ...
        label pairs for ((pair, pairid) in Vector.items(pair_config)) {
            ...
            switch(pair.config) {
                case (#icpswap({canister;})) {
                    ...
                    let ?info = Array.find(pd, func (x : ICPSwapRoot.PublicPoolOverView) : Bool {
                        x.pool == tcanister;
                    }) else continue pairs;
                    ...
                    let tvlUsd: Float = switch(Array.find(tvl, func (x : (Text, Float)) : Bool {
                        x.0 == tcanister;
                        })) {
                            case (?f) f.1;
                            case (null) 0;
        ...
        label pairs for ((pair, pairid) in Vector.items(pair_config)) {
            ...
            switch(pair.config) {
                case (#sonic({id})) {
                    ...
                    // find data in pd; 
                    let ?info = Array.find(pd, func (x : Sonic.PairInfoExt) : Bool {
                        x.id == id;
                    }) else continue pairs;
    ...
    private func icdex(pairid : PairId, canister: Principal) : async TickLast {
        ...
        let askSpread = Array.init<Nat8>(32, 0);
    ...
    ignore pipe3(
        ...
        mapAsync<[PairId], TickLast>(
            ...
            func(x:[PairId], next:(TickLast)->()) : async () {
            ...
            label pair for ( pairid in x.vals() ) { // make the calls 

                let pc = Vector.get(pair_config, pairid);
                let promise = switch(pc.config) { // Based on configuration, determine the async operation
                    case (#icdex({canister;})) {
            ...
            label awaitpair for ( (id, promise) in List.toIter(buf)) { // Await results of all promises
                try {
                let pairlast = await promise; // Await the promise to get the tick data
                next(pairlast); // Send the tick data to the next stage of processing
                } catch e {
        ...
        })).subscribe({ // Handle the output of the stream
        next = func(t) {
            let ft: Time.Time = first_tick; 
    ...
    private func token_dip20(tokenId: Nat, leder: Principal) : async ?TokenTickItem {
    ...
    private func token_icrc1(tokenId: Nat, leder: Principal, locking:TokenLocking) : async ?TokenTickItem {
        ...
        let circulating_supply : Nat = switch(locking) {
            ...
            case (#sns(cfg)) {
                ...
                neuron_mine(cfg, tokenId, sout, null, {circulating_supply; total_supply; fee; locking=null} );
    ...
    private func put_token_tick(ticks: Vector.Vector<TokenTick>, currentTick:Nat, tokenid:TokenId, t : TokenTickItem) : () {
         switch(Vector.getOpt<TokenTick>(ticks, currentTick)) {
                ...
                case(?p) {
                    ...
                    var psize = p.size();
                    if (tokenid >= psize) { // recreate larger array
    ...
    private func neuron_mine(cfg: SnsConfig, tokenId: Nat, sout: LockingVarTick, inc_last_id: ?SNSGov.NeuronId, tickInfo: TokenTickItem) : () {
    ...
    private func neuron_mine_internal(cfg: SnsConfig, tokenId: Nat, sout: LockingVarTick, inc_last_id: ?SNSGov.NeuronId, tickInfo: TokenTickItem) : async () {
        ...
        let gov : SNSGov.Self = actor(Principal.toText(cfg.governance));
            ...
            if (finished) {
                ...
                neuron_mine(cfg, tokenId, sout, last_id, tickInfo);
            }
    ...
    ignore pipe3(
        ...
        bufferTime<TokenId>(5, 3),  // every X seconds push Y items 
                                    // if you try more than 25 concurrent calls 
                                    // your playground canister will need more cycles
        mapAsync<[TokenId], (TokenId, TokenTickItem)>(
                                // you non-shared error when trying to return something generic from async
                                // instead we are providing a function where you can push it
            func(x:[TokenId], next:((TokenId, TokenTickItem))->()) : async () {

                                    // this can't be tucked inside a library, because of non-shared error
                                    // you can't have a List of <async T>
            var buf = List.nil<(TokenId, async ?TokenTickItem)>(); 
            label tloop for ( tokenid in x.vals() ) { // make the calls 

                let token = Vector.get(tokens, tokenid); // Get the token configuration

                let promise = switch(token.ledger) { // Based on configuration, determine the async operation
                    case (#icrc1({ledger})) {
            ...
            for ( (id, promise) in List.toIter(buf)) { // Await results of all promises
                try {
                    switch(await promise) {
                        case (?a) next(id, a); // Send the tick data to the next stage of processing
                        case (null) ();
                    };
                } catch e {
    ...
    ignore Timer.recurringTimer( #seconds 20, func() : async () {
    ...
    ignore Timer.recurringTimer( #seconds 43200, func() : async () {
        token_collect();
    });
    ...
    public query func oracles_get() : async [NodeInfoShared] {
        Array.map(BTree.toArray(nodes), func(x : (Principal, NodeInfo)) :  NodeInfoShared {
            {
                principal = x.0;
                good = x.1.good;
                bad = x.1.bad;
    ...
    private func oracle_buffer_digest() : () {
            ...
            label oraclestreams for ((id) in oracle_streams.vals()) {
                ...
                let stdinput = Array.mapFilter<OracleIncoming, Float>(udata, func(x:OracleIncoming) : ?Float {
                    // find if pair exists
                    let d = Array.find(x.data, func (y:(Text, Float)) : Bool {
                        y.0 == id;
                    });
                    switch(d) {
                        case (null) null;
                ...
                label reward for ({from; data} in udata.vals()) {
                    let d = Array.find(data, func (y:(Text, Float)) : Bool {
                        y.0 == id;
                    });
                    switch(d) {
    ...
    */
    ...
    public query func get_latest() : async [LatestTokenRow] {
        ...
        label tloop for ((token, tokenid) in Vector.items(tokens)) {
            ...
            if (tokenid != USD) switch(tokenPriceTrusted(tokenid, USD)) { // to usd directly
                case (?priceUsd) {
    ...
    public query func get_latest_extended() : async [LatestExtendedToken] {
        ...
        label tloop for ((token, tokenid) in Vector.items(tokens)) {
            ...
            if (tokenid != ICP) switch(tokenPrice(tokenid, ICP)) {
                case (?priceIcp) {
                    if (tokenid != XDR) Vector.add<LatestExtendedRate>(rez, {to_token =ICP; symbol = symbol # "/ICP"; rate = priceIcp.price; volume = priceIcp.volume / Icp2Usd.price; 
                    ...
                    Vector.add<LatestExtendedRate>(rez, {to_token = USD; symbol = symbol # "/USD"; rate = priceIcp.price * Icp2Usd.price; volume = priceIcp.volume; 
            ...
            if (tokenid != USD) switch(tokenPriceTrusted(tokenid, USD)) {
                case (?priceUsd) {
                    Vector.add<LatestExtendedRate>(rez, {to_token = USD; symbol= symbol # "/USD"; rate = priceUsd.price; volume = priceUsd.volume; 
            ...
            Vector.add<LatestExtendedToken>(alltokens, {
                id = tokenid;
                config = token;
                last;
    ...
    ignore Timer.recurringTimer( #seconds 60, func() : async () {
        label pairs for ((pair, pairid) in Vector.items(pair_config)) {
            if (pair.deleted == true) continue pairs;
            switch(pair.config) {
                case (#xrc(xrc_pairs)) {
